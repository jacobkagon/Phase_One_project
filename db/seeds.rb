...
jennifers_body = Plan.find_or_create_by(boss_id: gru.id, job: "Kill all brother's of enemy's Mob.")
batmans_evil_twin = Plan.find_or_create_by(boss_id: darth_vader.id, job: "Help Batman's evil twin take down Gotham City")
annihilate_austin_powers = Plan.find_or_create_by(boss_id: dr_evil.id, job: "Squash Austin Powers like the bug he is.")
drown_london = Plan.find_or_create_by(boss_id: gru.id, job: "Drown London with a pot of boiling spaghetti and spicy marinara sauce")
destroy_number2 = Plan.find_or_create_by(boss_id: dr_evil.id, job: "Kill number 2, get the job done!")
set_undead_sirens_free = Plan.find_or_create_by(boss_id: ursula.id, job: "Set undead sirens free upon the beaches of Florida")
...
medusa_job_2 = PlanHenchman.find_or_create_by(plan_id: set_undead_sirens_free.id, henchman_id: medusa.id)
